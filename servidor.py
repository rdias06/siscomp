...
def enviardictdeconn(conn):
    i = 0
    global connparamensagem
    while i < 50:
        conn.sendall(connparamensagem)
        i = i + 1
    if i == 10:
        enviardictdeconn(conn)

def aceitar(conn):
    identificador1 = 0
    identificador2 = 0
    global usuariosonlineadd , usuariosonlineamsg , connparamensagem, connparalista
    while True:
        ...
        if int(numeros == 3):
            ...
            if dados[0] == 'recebemensagem':
                ...
                try:
                    ...
                    print('erro')
            if dados[0] == 'recebelista':
                print('guardando conn para usuario receber lista de quem esta online')
                try:
                    bancodedados = open("usuarios.txt", 'r')
                    bancodedados2 = open("usuarios2.txt", 'w')
                    bancodedados2.close()
                    for linha in bancodedados:
                        ...
                        nome, senha, email, connenviar = linha.split(";")
                        if (nome == dados[1]) and (senha == dados[2]):
                            connparalista[email] = conn
                            print('conn para enviar dicionario de ' + email + ' guardado')
                            conn.sendall('ok')
                except Exception:
                    print('erro guardando conn para enviar dicionario')
                enviardictdeconn(conn)
