...
def thread3():
    class mensagem:
        ...
        def recebermensagem(self):
            i = 1
            global nome, senha, login, dictdeconns
            login = 0
            while i > 0:
                if login == 1:
                    l = []
                    l.append('recebelista')
                    l.append(nome)
                    l.append(senha)
                    sep = ';'
                    info = sep.join(l)
                    ...
                    if confirmacao == 'ok':
                        print('conn para receber lista guardado.\n')
                        while True:
                            try:
                                dados = s.recv(1024)
                                dictdeconns = {}
                                dictdeconns = dados.split(';')
                            except Exception:
                                print('erro')
                else:
                    i = i + 1
    ...
    class usuario:
        ...
        def mandarmensagem(self):
            ...
            while i < 50 :
                ...
                for linha in bancodedados:
                    linha = linha.strip("\n")
                    identificador, emailamigo, info = linha.split(";")
                    i = i + 1
                    if dictdeconns.has_key(emailamigo):
                        l = []
                        l.append(identificador)
                        l.append(emailamigo)
                        l.append(info)
                        ...
                        envio = sep.join(l)
                        s.sendall(str(envio))
                        print(emailamigo + ' está online agora, sua mensagem foi encaminhada para ele.\n')
            if i == 10 :
                u.mandarmensagem()
        ...
        def mensagemamigo(self):
            ...
            if confirmacao == 'amigooffline':
                ...
                bancodedados.write(info)
                bancodedados.write('\n')
                bancodedados.close()
                t4 = Thread(target=u.mandarmensagem, args=())
                t4.start()
                t5 = Thread(target=u.menu2, args=())
                t5.start()
            if confirmacao == 'naocadastrado':
...
if __name__ == '__main__':
    global nome, senha, login, mensagemoffline, dictdeconns
    mensagemoffline= []
